...
GoriachevaKMultSparseComplexMatrixCcsOMP::GoriachevaKMultSparseComplexMatrixCcsOMP(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool GoriachevaKMultSparseComplexMatrixCcsOMP::ValidationImpl() {
  auto &[a, b] = GetInput();
...
bool GoriachevaKMultSparseComplexMatrixCcsOMP::PreProcessingImpl() {
  GetOutput() = {};
  return true;
}
  ...
  for (int j = 0; j < b.cols; j++) {
    ...
    for (int r : used_rows) {
      if (accumulator[r] != Complex(0.0, 0.0)) {
        local_rows[j].push_back(r);
        local_values[j].push_back(accumulator[r]);
      }
    }
  }
...
bool GoriachevaKMultSparseComplexMatrixCcsOMP::PostProcessingImpl() {
  return true;
}
