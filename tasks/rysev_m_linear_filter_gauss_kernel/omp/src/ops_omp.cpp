...
RysevMGaussFilterOMP::RysevMGaussFilterOMP(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = 0;
}

bool RysevMGaussFilterOMP::ValidationImpl() {
  return GetInput() >= 0 && GetOutput() == 0;
}

bool RysevMGaussFilterOMP::PreProcessingImpl() {
  if (GetInput() == 0) {
    std::string abs_path =
...
bool RysevMGaussFilterOMP::RunImpl() {
  int rows = height_;
  int cols = width_;
  int ch = channels_;
...
bool RysevMGaussFilterOMP::PostProcessingImpl() {
  int64_t total = 0;
